function WMImage = ImprovedSpreadSpectrumEmbed_EmbedPos(InputImage, watermark, pattern, alpha, lambda, blkSize, embedPos)
...
%     end
	...
	for i = 1 : floor(height/blkSize)
        ...
        for j = 1 : floor(width/blkSize)
            ...
            s = dctX + (alpha*b - lambda*dot(dctX, pattern)/dot(pattern, pattern)) * pattern;
